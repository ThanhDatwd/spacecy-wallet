...
const auctionApi = {
	...
	//makeBid
	createMakeBid(data: any, id: any): Promise<Response<any>> {
		const url = `auctions/bid/auctionId/${id}`;
		return axiosClient.post(url, data);
	},
	//settle
	settleAuction(data: any, id: any): Promise<Response<any>> {
		const url = `auctions/settle/auctionId/${id}`;
		return axiosClient.post(url, data);
	},
