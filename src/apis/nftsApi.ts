...
const nftsApi = {
	...
	getListTokenIdTest(
		...
		const { pageSize, page } = paginationParams;
		const url = `/items/query1/pageSize/${pageSize}/page/${page}`;
		const body = { ...filter };
		return axiosClient.post(url, body);
	},
	...
	getDetailNftItemById(filter: ListParams): Promise<Response<any>> {
		const { itemId, userAddress } = filter;
		const url = `/items/detail/itemId/${itemId}?userAddress=${userAddress}`;
		return axiosClient.get(url);
	},
	...
	//Static
	getStaticData(): Promise<Response<any>> {
		const url = `/items/static`;
		return axiosClient.get(url);
	},
};
