		...
		if (item) {
			if (item.status === 'live') {
				return (
					...
					>
						<CountDown
							timeStart={item.startTime * 1000}
							timeEnd={item.endTime * 1000}
		...
		<Box ref={observe} sx={{ p: '12px' }}>
			{
				...
				!isLoading ? (
					!isSuccess ? (
						...
						<LinkWrapper
							href={`#${PATH_AUCTION.detail}/${item._id}`}
							sx={{
								transition: 'all 0.5s',
								'&:hover': {
							...
							<ItemWrapperIGO>
								...
								<TextWrapperIGO>
									...
									<GridWrapperIGO>
										...
										>
											{/* <Typography variant="body2">
												{formatTimeHistory(new Date(item.endTime * 1000))}
											</Typography> */}
											{/* <Countdown
												// className="countDown"
												timeStart={item.startTime * 1000}
										...
										{/* <Stack direction="row">
											...
											<Stack direction="row" gap="4px" alignItems="center">
												<Typography variant="body2">
													{item.items[0].ownerInfo[0].username}
												</Typography>
												<Box>
													<img
