...
export default function CreateAuction() {
	...
	const onSubmit = async (data: IFormCreateAuctionInputs) => {
		...
		if (data.paymentToken === '0x0000000000000000000000000000000000000000') {
			...
			priceToWei = await erc20function().changeTokenToWei(data.paymentToken, data.minPrice);
		}
	...
	const handleStep1 = async (isCheck: boolean): Promise<void> => {
		...
		const data: ApproveAuction = {
			...
			auctionAddress: CONTRACT[97].MetaSpacecyAuction,
		};
	...
	const handleStep2 = async (): Promise<void> => {
		...
		const newData = {
			...
			collectionId: inoDetail.collectionId,
		};
		let isCompleted;
		await auctionApi
			.createAuction(newData)
			.then((res) => {
				if (res) {
					isCompleted = true;
				}
