...
export default function CreateOrEditCollection(props: CreateCollectionProps) {
	...
	const onSubmit = async (data: IFormAddOrEditCollectionInputs) => {
		...
		let logoURL: uploadURL = { result: '', fileName: '' };
		let backgroundURL: uploadURL = { result: '', fileName: '' };
		...
		try {
			...
			if (!isEdit) {
				const newData: CreateCollectionInput = {
					...data,
					logo: logoURL.result,
					fileLogoName: logoURL.fileName,
					background: backgroundURL.result,
					fileBackgroundName: backgroundURL.fileName,
					userAddress: address!,
