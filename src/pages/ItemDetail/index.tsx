...
import { selectNftItem, resetAll } from 'redux/slices/nftItemByItemIdSlice';
...
import { setChainId } from 'redux/slices/web3InfoSlice';
//actions
...
const ItemDetail = () => {
	...
	const addressState: any = useSelector(selectAddress);
	//useEffect
	// fetch NFT detail
	console.log('message sss', item);
	useEffect(() => {
		if (itemId) {
			dispatch(fetchDetailNftItemById(addressState, itemId, executeAfterFetchNftItemById));
		}
	...
	const followNetworkOfItem = (itemChainId: number, currentChainId: number) => {
		if (itemChainId === currentChainId) {
			...
			if (itemChainId === 56) {
				...
				console.log('ether');
				changeNetwork(mainnetEthereum, 1);
	...
	return (
		<Container maxWidth="xl" sx={{ mt: 15, fontStyle: 'italic' }}>
			...
			{/* Item detail tabs */}
			<Box sx={{ mt: 5 }}>
				<TabItemDetail listActivityPriceChart={listActivityPriceChart} item={item} />
			</Box>
