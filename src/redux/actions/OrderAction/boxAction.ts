...
export function BoxAction() {
	...
	const BuyBox = async (data: BuyBoxInput): Promise<boolean> => {
		...
		try {
			...
			await contractWormhole.methods.buyBox(optionId, amount).send({ from: userAddress });
	...
	const BuyTicketCard = async (data: any): Promise<boolean> => {
		...
		try {
			...
			await contractWormhole.methods.buyTicketCard(amount).send({ from: userAddress });
	...
	const HarvestItem = async (data: any): Promise<boolean> => {
		...
		try {
			...
			await contractMetaSpacecyStaking.methods.harvest(slotIndex).send({ from: userAddress });
	...
	const UpgradeItem = async (data: any): Promise<boolean> => {
		...
		try {
			...
			await contractWormhole.methods.upgrade(toTokenId, 1).send({ from: userAddress });
