...
export function fetchListOrderOfferMade(filter: ListParams, MyCallBack?: Function) {
	return async () => {
		dispatch(startLoading());
		try {
			const response: ListResponse<OrderResponseAPI> = await orderApi.getListOfferReceive(
				filter
			);
			dispatch(getListOrderOfferSuccess(response));
		} catch (error) {
