...
export const formatTimeHistoryCustom = (time: Date | string | number): string => {
	time = new Date(time);
...
export const formatDayMonth = (time: Date | number | string): string => {
	let result: string;

	if (!time) {
		result = '-----';
	} else {
		result = moment(time).format('l');
	}
	return result;
};
export const formatTimeHours = (time: Date | number | string): string => {
	let result: string;

	if (!time) {
		result = '-----';
	} else {
		result = moment(time).format('LT');
	}
	return result;
};
